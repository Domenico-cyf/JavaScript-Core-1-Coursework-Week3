...
*/
function isAccessibleByTransportMode(array2, string) {
  if (array2.find(element => element === string)) {
    return true;
  } else { return false;}
}
...
function journeyPlanner(locations, transportMode) {
 ...
 let location = locations.filter(element => element.includes(transportMode)).map(element => element[0])
return location
}
